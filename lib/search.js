...
function passesFilters(post, filters) {
  const { tags, dateRange, readingTime, author } = filters
  ...
  // Reading time filter
  if (readingTime && post.readingTime) {
    const minutes = post.readingTime.minutes || 0
    if (readingTime.min && minutes < readingTime.min) return false
    if (readingTime.max && minutes > readingTime.max) return false
  }
